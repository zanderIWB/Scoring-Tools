...
def main():
    ...
    df = pd.read_excel("Test_Database2.xlsx", engine="openpyxl")

    no_tickbox_columns = ["Product", "Type"]
    non_numeric_columns = ["Product", "Type"]
    bad_columns = ["Cost of Production"]

    # Check for any columns that appear in both non_numeric_columns and bad_columns
    common_columns = set(non_numeric_columns).intersection(set(bad_columns))
    if common_columns:
        st.error(
            f"The following columns are listed both in non_numeric_columns and bad_columns: {', '.join(common_columns)}"
        )
        return
    ...
    num_columns = 3
    num_rows = math.ceil(len(tickbox_columns) / num_columns)
    columns_per_row = math.ceil(len(tickbox_columns) / num_rows)

    cols = []
    for _ in range(num_rows):
        cols.append(column_expander.columns(columns_per_row))

    for i, column in enumerate(tickbox_columns):
