...
def main():
    ...
    analytics_columns = ["Net Assets", "Discount", "Dividend Yield", "Charge (no perf fee)", "Charge (w/ perf fee)", "Net Cash"]
    
    
    bad_columns = ["Top 5 Weight", "Top 10 Weight", "Discount", "Charge (no perf fee)", "Charge (w/ perf fee)", "Net Cash"]
    ...
    tickboxes("Operational", 0, no_of_operational)
    tickboxes("Performance", no_of_operational, no_of_performance)
    tickboxes("Consistency", (no_of_operational + no_of_performance), no_of_consistency)
    tickboxes("Diversification", (no_of_operational + no_of_performance + no_of_consistency), no_of_diversification)
    tickboxes("Analytics", (no_of_operational + no_of_performance + no_of_consistency + no_of_diversification), no_of_analytics)
    ...
    if contains_similar_element(consistency_columns, selected_columns):
    	...
    	filtered_df.insert(consistency_column_index + 1, "Average Rank", rank_df.mean(axis=1))
    	
    	if score_columns[0] == ["Performance Score"]:
    		score_columns = score_columns[0] + ["Average Rank"] + score_columns[1:]
    	else:
    		score_columns = ["Average Rank"] + score_columns
    ...
    if contains_similar_element(diversification_columns, selected_columns):
        ...
        for i, col in enumerate(common_columns):
            if col in bad_columns:
            	...
            	normalized_common[i] = (filtered_df[col] - filtered_df[col].min()) / (
                	filtered_df[col].max() - filtered_df[col].min())
        div_scores = normalized_common.mean(axis=1)
        for i, col in enumerate(filtered_df):
        ...
        if score_columns[0] == ["Performance Score"] and score_columns[1] == ["Average Rank"]:
        	score_columns = score_columns[0:2] + ["Diversification Score"] + score_columns[2:]
        elif score_columns[0] == ["Performance Score"] or score_columns[0] == ["Average Rank"]:
        	score_columns = score_columns[0] + ["Diversification Score"] + score_columns[1:]
        else:
