...
def main():
    ...
    for i, column in enumerate(selected_columns[1:]):
        normalized_column = (filtered_df[column] - filtered_df[column].min()) / (filtered_df[column].max() - filtered_df[column].min())
    ...
    filtered_df["Score"] = filtered_df["Score"].apply(lambda x: str(x).rstrip("0").rstrip(".") if isinstance(x, float) else x)
